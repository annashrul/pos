...
class M_grafik extends CI_Model{
  ...
  function penjualan_perbulan(){
      $query = $this->db->query("SELECT DATE_FORMAT(jual_tanggal,'%d') AS tgl,COUNT(jual_total) AS total FROM tbl_jual WHERE MONTH(jual_tanggal)=MONTH(CURDATE()) GROUP BY DATE(jual_tanggal)");
      if($query->num_rows() > 0){
        foreach($query->result() as $data){
          $hasil[] = $data;
        }
